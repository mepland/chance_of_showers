...
def plot_func(plot_objs_in, col_x, col_y, fig_size=(6,5)):
    ...
    for k,plot_obj in plot_objs.items():
        if plot_obj['type'] == 'scatter':
            _ = ax.plot(plot_obj['x'], plot_obj['y'], marker=plot_obj['ms'], linestyle=plot_obj.get('ls', 'None'), color=plot_obj['c'], label=plot_obj['label'])
    ...
    for k,plot_obj in plot_objs.items():
        if plot_obj['type'] == 'linear_fit':
            ...
            _ = ax.plot(x, y, marker='None', linestyle=plot_obj.get('ls', '-'), color=plot_obj['c'], label=plot_obj['label'])
            plot_objs[k]['leg_object'] = _[0]
    ...
    leg = ax.legend(leg_objects, [ob.get_label() for ob in leg_objects], fontsize=10, bbox_to_anchor=(1.02, 1), ncol=1, borderaxespad=0.)
